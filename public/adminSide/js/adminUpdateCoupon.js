$("#updateForm").submit((e) => {
  ...
  axios
    .put($("#updateForm").attr("action"), data)
    .then((res) => {
      if (res.data.status) {
    ...
    .catch((err) => {
      if (err.response.data.status) {
        return (location.href = err.response.data.url);
      }
...
discount.addEventListener("keyup", () => {
  if (
    (Number(discount.value) < 100 && Number(discount.value) >= 0) ||
    !discount.value
  ) {
...
fPrice.addEventListener("keyup", () => {
  if (Number(fPrice.value) >= Number(lPrice.value)) {
    if (lPrice.value) {
      discount.value = `${
        ((fPrice.value - lPrice.value) / fPrice.value) * 100
      }`;
    } else {
      discount.value = `0`;
    }
    ...
    for (let i = 0; i < inputFiles.length; i++) {
      if (
        !validImageExtensions.includes(
          inputFiles[i].name.substring(inputFiles[i].name.lastIndexOf("."))
        )
      ) {
        document.querySelector("#fileDiv").classList.add("errDiv");
        document.querySelector(
          "#err"
        ).innerHTML = `Please select ${validImageExtensions
          .toString()
          .split(",")
          .join(", ")} images Only`;
        continue;
